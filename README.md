...
- Persistent disk cache for LLM-generated assessments (`assessor/cache.py`) — keyed on `(title, description, diff, provider, model)`; a sidebar lists past runs for one-click reload without re-running the graph. Selecting a sample PR that's already cached shows the result immediately — no need to click Assess Impact just to see a run that already happened; a `📦 Loaded from a previous run` caption marks it so it's never mistaken for a fresh call. A cache miss just pre-fills the form as before. Backed by two layers: the runtime cache (gitignored, wiped by every Space rebuild) and a git-tracked `data/seed_assessments/` bundle for the 5 sample PRs, so a freshly deployed Space isn't starting from zero.
...
- The gate also fires for a HIGH-risk assessment loaded from cache or the bundled seed data, not just a live paused run — `human_approved` is the single source of truth the UI checks, regardless of how the assessment got into session state. There's no live graph thread to resume in that case, so Acknowledge just applies the same field flip `await_approval` would, for that viewing session only; it's never written back to disk, so the next visitor (or the next reload) sees the gate fresh rather than the first person to click it silently clearing it for everyone. (Originally this only checked for a live paused thread, which meant nobody visiting the deployed demo and clicking a HIGH-risk sample PR ever saw the gate at all — fixed after the seed-data feature below made cache-loading the common path, not the exception.)
- `graph.py`'s checkpointer is an in-memory `MemorySaver`, one per Streamlit session — a pending approval on a **live** run does not survive an app restart (see **Known limitations**)
...
- The persistent cache (`assessor/cache.py`) is local-disk. On Hugging Face Spaces' free tier it survives a simple restart but not a rebuild (triggered by every push) — a change you paste yourself won't stay cached across a redeploy. The 5 sample PRs are the exception: `data/seed_assessments/` ships git-tracked, pre-computed results for them (same lookup key as the runtime cache), so a freshly deployed Space still has something to show immediately instead of every visitor needing to pay for a live LLM call first. 4 of 5 are seeded as of this writing — `pr_005` is missing because generating it hit the backend's daily rate limit (see below); it behaves exactly as before (click Assess Impact) until re-seeded.
- The demo's OpenAI-compatible backend (chatanywhere.tech free tier) caps at **30 requests/day per API key**, shared across every caller — my own testing, the deployed Space, and any visitor's live runs all draw from the same quota. Hitting it mid-session returns a 429 the UI surfaces as a normal "Assessment failed" error, not a crash, but it does mean the live demo can go quiet for the rest of the day if it gets enough traffic (or enough of my own re-testing).
...
| `data/seed_assessments/` | Bundled pre-computed cache for the sample PRs — survives a fresh deploy |
